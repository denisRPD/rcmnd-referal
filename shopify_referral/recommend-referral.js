console.debug("Rmd ScriptTag loaded");
...
(async () => {
  ...
  function getReferralParam() {
    ...
    if (params.has(referralParamName)) {
      console.debug("Referral query present");
  ...
  function init() {
    ...
    if (referralParam) {
      console.debug("Referral param present");
      referral = referralParam;
      console.debug("Setting new param to the cookie");
      Cookies.set(referralCookieName, referralParam);
    } else {
      console.debug("Referral param not present");
      // missing referral param but we have cookie available
      if (referralCookie) {
        console.debug("Referral cookie present");
        referral = referralCookie;
      }
    }

    if (referral) {
      console.debug("Setting referral to cart");
      const attrs = {};
      attrs[orderAttributeName] = referral;
      setCartAttributes(attrs);
    } else {
      console.debug("Referral not present - no cart interaction");
    }
  }
